...
def generate_random_amplitude_envelope(t: np.ndarray, rng=None):
    ...
    - random tau in [0.5, 2.5] for smooth (non-aggressive) transitions,
    - 50% tension spline (smooth) vs. 50% step function (abrupt).
    """
    ...
    # Controlled amplitude (1–8) to achieve more natural variations without extreme peaks
    ...
    # 50% spline tension (smooth transitions) vs. 50% step function (abrupt variations)
    use_tension = rng.choice([True, False], p=[0.5, 0.5])
    if use_tension:
        # tau in [0.5, 2.5] to ensure smooth transitions without extreme oscillations
